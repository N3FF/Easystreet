...
public class EasyStreetGUI extends JFrame implements ActionListener {
    ...
    /**
     * The offset to use to position cross walk lines and to use for cross walk
     * light size.
     */
    ...
    private void advanceAnimation() {
        for (final Vehicle v : myVehicles) {
            ...
            for (final Vehicle other : myVehicles) {
                if (v == other) { // use of == is intentional - checking for
                                  // same object
    ...
    private boolean isValidIndex(final int theY, final int theX) {
        return 0 <= theY && theY < myGrid.length && 0 <= theX && theX < myGrid[theY].length;
    }
            ...
            for (int x = 0; x < myGrid[y].length; x++) {
                ...
                switch (myGrid[y][x]) {
                    ...
                    case CROSSWALK:
                        ...
                        // draw a small circle of appropriate color centered in
                        // the square
                        setLightPaint(theGraphics);
     ...
     * @param theY The y-coordinate of the street.
     */
    private void drawStreetLines(final Graphics2D theGraphics, final int theX,
                                 final int theY) {
     ...
     */
    private void drawCrossWalkLines(final Graphics2D theGraphics, final int theX,
                                    final int theY) {
        ...
        if (isValidIndex(theY, theX - 1) && myGrid[theY][theX - 1] == Terrain.GRASS) {
            ...
            theGraphics.drawLine(leftx, topy + (int) (SQUARE_SIZE * CROSSWALK_SCALE), rightx,
                                 topy + (int) (SQUARE_SIZE * CROSSWALK_SCALE));
    ...
    private class EasyStreetPanel extends JPanel {
        ...
        public void paintComponent(final Graphics theGraphics) {
            ...
            for (final Vehicle v : myVehicles) {
                ...
                g2.drawImage(img, v.getX() * SQUARE_SIZE, v.getY() * SQUARE_SIZE, SQUARE_SIZE,
                             SQUARE_SIZE, this);
